# Station 6 blueprint v2 (AI Assistance for Computational Thinking)

**Created:** January 10, 2026 at 8:25 PM ET
**Updated:** January 10, 2026 at 8:40 PM ET (v2 - removed accessibility focus, replaced with code quality/correctness)
...
- Vague prompts produce generic, low-quality code—detailed requirements get professional results
- AI doesn't understand context without explicit guidance
...
- Code quality requires explicit constraints in prompts
...
1. **"I asked AI to generate quotes and used them immediately without checking the schema."**
   When I ran `npm run validate`, everything failed. The AI had used wrong category names, inconsistent tone values, and duplicate IDs. I had to regenerate everything with explicit schema requirements. That taught me: AI doesn't know your project's rules unless you tell it.

2. **"I thought 'generate good quotes' was enough detail in my prompt."**
   AI gave me generic motivational quotes that had nothing to do with computational thinking. When I provided specific requirements (category, tone, word count, theme), the output actually matched what Magic Quotes needed. Detailed prompts get detailed results.

3. **"I copied AI-generated code without understanding what it did."**
   It looked right, but when Prof. Teeters asked me to explain why I chose those specific quotes, I couldn't answer. She reminded me: "Speed without understanding is technical debt." Now I verify AND understand before using anything.

4. **"I assumed AI would follow the JSON structure automatically."**
   My first AI-generated output was valid JSON but didn't match Magic Quotes' schema at all. When I explicitly provided the required fields and format, AI delivered exactly what I needed. AI responds to what you ask for—nothing more.
...
- Schema-focused prompts vs. generic prompts
...
- Every AI-generated example must be verified against schema
- Focus on responsibility, not AI capabilities

**Student interaction:**

- **What they watch HAP do:** Create quote-hap.json with AI, verify schema compliance, commit via VS Code GUI, push to fork, open PR
...
| Schema validation breakthrough | Realization of importance   | Understanding responsibility   | hap-waving_dgzacg       |
...
5. **The Schema Problem: AI's Biggest Blind Spot** - Why AI doesn't know your project's rules without explicit guidance
6. **Generating quote-hap.json with AI** - Step-by-step with schema-first prompting
7. **Verification Workflow: Checking AI Output** - JSON validation, schema testing, manual review
...
**HAP's realization:**
"AI can generate code faster than I can type, but it doesn't understand my project's requirements unless I explain them. Prof. Teeters taught me: you own every line of code, whether you wrote it or AI did."
...
**HAP's discovery:**
"AI consistently ignores project-specific rules unless I explicitly provide them. EVERY prompt must include schema requirements. It's not optional—it's how you ensure code works correctly."

**Teaching approach:**

- Show AI output WITHOUT schema requirements → validation fails
- Show AI output WITH schema requirements → validation passes
- Make schema the first requirement in every prompt template
...
- Show verification checklist (schema validation, JSON structure, quote quality)
...
**Result:** Generic, non-specific, doesn't match schema, poor quality
...
**Result:** Professional, schema-compliant, on-theme, properly formatted

### Schema-first template

```text
Create [task] following this EXACT schema:
...
[Then add your other requirements...]
```
...
1. **Students understand AI responsibility**
   ...
   - Schema compliance must be explicitly requested
...
4. **Students internalize the 6 AI rules**
   - Schema compliance first, always
   - Detailed prompts required
   - Verification mandatory
   - Understanding required
   - Quality needs explicit constraints
...
**Schema/quality emphasis:**

- Schema compliance appears in EVERY section
- Not optional or secondary—foundational
- AI's weakness (not knowing project rules) becomes the teaching opportunity
...
- Verify schema compliance manually
...
1. **Always Specify Schema Requirements First**
   AI doesn't know your project's rules unless you provide them. Start EVERY prompt with schema constraints before other requirements.

2. **Detailed Prompts Get Professional Results**
   "Generate quotes" produces junk. "Generate 5 reflective developer-wisdom quotes, 10-20 words each, matching schema fields X/Y/Z, focused on computational thinking" produces quality.

3. **Verify Everything Before Committing**
   Run validation tools. Test manually. Check schema compliance. If you can't explain what the code does, you don't own it yet.
...
5. **Quality Requires Explicit Constraints**
   AI won't optimize unless you ask. Include quality constraints in prompts: "concise," "specific," "on-theme," "plain language."

6. **You Own Every Line of Code**
   Whether you wrote it or AI did, you're responsible for quality, correctness, and schema compliance. Blame doesn't transfer to the AI.
...
- [x] Schema-first approach is emphasized throughout
...
**Blueprint status:** Complete and ready for implementation (v2 - accessibility removed, replaced with schema/quality focus)
